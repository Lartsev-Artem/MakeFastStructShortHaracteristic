...
int main(int argc, char* argv[])
{
	...
	if (ReadStartSettings(name_file_settings, name_file_vtk, name_file_sphere_direction, out_file_grid_vtk, BASE_ADRESS)) {
...
//-------------------читающиес€ файлы, построенные в build_graph---------------------------
	const std::string name_file_graph = BASE_ADRESS + "graph.bin";
...
//--------------------------------создающиес€ файлы----------------------------------------
	...
	if (ReadFileVtk(0, name_file_vtk, unstructured_grid, foo, foo, foo, true)) {
	...
	WriteIdPairs(name_file_pairs, unstructured_grid);
	
	const int count_cells = 0;
	const int count_directions = 0;
	return 0;
	...
	fread_unlocked(sorted_id_cell.data(), sizeof(int), count_cells * count_directions, f);
	fclose(f);
...
//==========================================================
			...
			fwrite_unlocked(&shift_out, sizeof(int), 1, file_shift_out);  shift_out = posOut;
			fwrite_unlocked(&shift_res, sizeof(int), 1, file_shift_res); shift_res = posRes;
